<?php
  ...
  if(isset($_POST['submit'])){
    ...
    if($_FILES['image']['name']!=''){
      ...
      if($image_size<1000000){
        if($image_ext == 'jpg' || $image_ext == 'png'|| $image_ext == 'jpeg'){
          if(move_uploaded_file($image_tmp,$image_path)){
            $sql = "insert into CmsSystem(`title`, `description`,`image`,`catagory`,`author`,`status`) values('".$title."','".$_POST['description']."','".$image_db_path."','".$_POST['catagory']."','".$_SESSION['user']."','".$_POST['status']."')";
            if(mysqli_query($conn,$sql)){
              header('Location: post_list.php');
            }
            else{
              $error = "<div class='alert alert-danger'>Post not uploaded</div>";
            }
          }
          else{
      ...
      $sql = "insert into CmsSystem(`title`, `description`,`catagory`,`author`,`status`) values('".$title."','".$_POST['description']."',".$_POST['catagory'].",'".$_SESSION['user']."','".$_POST['status']."');";
      echo $_POST['status'];
      echo $sql;
      ...
      <div class="container-fluid">
        <form class="form-horizontal text-dark" action="new_post.php" method="post" enctype="multipart/form-data">
            ...
            <select id="catagory" class="form-control" name="catagory" required>
              ...
              <?php
                ...
                while($rows=mysqli_fetch_assoc($run)){
                  echo '<option value="'.$rows['c_id'].'">'.$rows['catagory_name'].'</option>';
                }
              ?>
          ...
          <div class="form-group mt-3">
            ...
            <input id="Image" class="form-control btn btn-primary " type="file" name="image"></input>
          </div>
